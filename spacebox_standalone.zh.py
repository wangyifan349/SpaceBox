...
- 应用配置不会从环境变量中读取。
- SQLite 数据库和持久化 Session 密钥会自动管理。
...
class Media(Base):
    ...
    # v0.1 遗留字段。新上传内容改为存储在 content_data 中。
...
class EmbeddedTemplates:
    """与下方 TemplateResponse 调用兼容的小型 Jinja 适配器。"""
...
def ensure_schema() -> None:
    ...
    # 将旧版 v0.1 文件系统附件迁移到 SQLite BLOB 存储。
    with engine.begin() as connection:
...
async def read_upload_payload(upload: UploadFile) -> MediaUploadPayload | None:
    ...
    if mime_type.startswith("image/") and file_extension in ALLOWED_IMAGE_EXTENSIONS:
        ...
        raise HTTPException(
            ...
            detail=(
                f"不支持的文件类型：{upload.filename}. "
                "仅允许常见图片格式以及 MP4/WebM/MOV 视频。"
            ),
        )
        ...
        while True:
            ...
            if len(file_buffer) > MAX_MEDIA_FILE_SIZE_BYTES:
                raise HTTPException(
                    ...
                    detail=f"文件 {upload.filename} 超过 30 MB",
                )
    finally:
        await upload.close()
...
def register(
    ...
    if not username_length_is_valid or not username_characters_are_valid:
        flash(request, "用户名必须为 3–32 个字符，只能使用字母、数字或下划线。", "danger")
        return redirect_to("/register")
    if (
        not normalized_display_name
        or len(normalized_display_name) > MAX_DISPLAY_NAME_LENGTH
    ):
        flash(request, "显示名称为必填项，且不得超过 64 个字符。", "danger")
        return redirect_to("/register")
    if session.scalar(
        select(User).where(User.username == normalized_username)
    ):
        flash(request, "该用户名已被占用。", "danger")
        return redirect_to("/register")
    ...
    csrf_token(request)
    flash(request, "注册完成。欢迎来到 SpaceBox！", "success")
    return redirect_to("/")
...
def login(
    ...
    if authenticated_user is None or not verify_password(
        ...
        flash(request, "用户名或密码不正确。", "danger")
...
async def create_post(
    ...
    if not normalized_content.strip() and not uploaded_files:
        flash(request, "帖子必须包含文字或至少一个附件。", "danger")
        return redirect_to("/post/new")
    if len(normalized_content) > MAX_POST_LENGTH:
        flash(request, "帖子内容不得超过 5,000 个字符。", "danger")
        return redirect_to("/post/new")
    if len(uploaded_files) > MAX_MEDIA_FILES_PER_POST:
        flash(
            request,
            f"每篇帖子最多可以包含 {MAX_MEDIA_FILES_PER_POST} 个附件。",
            "danger",
        )
        return redirect_to("/post/new")
    ...
    if scheduled_datetime:
        flash(request, "帖子已成功设置定时发布。", "success")
    else:
        flash(request, "帖子发布成功。", "success")
...
def create_comment(
    ...
    if (
        ...
        flash(request, "评论不能为空，且不得超过 1,000 个字符。", "danger")
...
def follow_user(
    ...
    flash(request, f"现在正在关注 @{target_user.username}", "success")
...
def unfollow_user(
    ...
    session.commit()
    flash(request, f"已取消关注 @{target_user.username}", "success")
    return redirect_to(f"/u/{target_user.username}")
...
def settings_update(
    ...
    if len(normalized_bio) > MAX_BIO_LENGTH:
        flash(request, "个人简介不得超过 500 个字符。", "danger")
        return redirect_to("/settings")
