#!/usr/bin/env python3
"""SpaceBox standalone single-file launcher.

This file is intentionally designed to be fully self-contained:
- The FastAPI backend, SQLAlchemy models, authentication, and routes all live here.
- Jinja templates and project CSS/JS are embedded here; Bootstrap is loaded from an external CDN.
- Application configuration is not read from environment variables.
- The SQLite database, media directory, and persistent session secret are managed automatically.
- Large media uses streamed file storage and HTTP Range responses so entire files are never loaded into memory.
- Every startup removes stale empty accounts and reconciles the media file index.

Run directly:``python spacebox_standalone.en.cdn.media.py``
"""

# Runtime dependency checks must run first, so third-party modules are intentionally imported afterward.
# ruff: noqa: E402
...
def _ensure_runtime_dependencies() -> None:
    """Install missing runtime dependencies when this file is run standalone."""
    ...
    # Some managed Python installations make pip fall back to the user site, which may not be on sys.path yet.
    if user_site_packages not in sys.path and Path(user_site_packages).is_dir():
        sys.path.insert(0, user_site_packages)
...
class MediaFormat:
    """An uploadable media format and its server-side limits."""
...
def format_byte_size(byte_count: int) -> str:
    """Format a byte count using binary units suitable for UI messages."""
    units = ("B", "KB", "MB", "GB", "TB")
...
# First-party templates, styles, and scripts remain readable source; third-party frontend libraries are loaded via CDN.
...
{% block title %}Error 路 SpaceBox{% endblock %}
{% block content %}
...
{% block title %}Feed 路 SpaceBox{% endblock %}
{% block content %}
<div class="d-flex justify-content-between align-items-center mb-4 gap-3">
  <div>
    <h1 class="h4 mb-1">Feed</h1>
    <div class="text-secondary small">Public posts and content you are allowed to view. Times are shown in your browser's local time zone.</div>
  </div>
  {% if current_user %}
...
{% else %}
  <div class="empty-state card border-0 shadow-sm text-center py-5 text-secondary">
    <div class="fs-2 mb-2">馃獝</div>
    <p class="mb-0">There are no posts you can view yet.</p>
  </div>
{% endif %}
{% endblock %}""",
    "login.html": r"""{% extends "base.html" %}
{% block title %}Log in 路 SpaceBox{% endblock %}
{% block content %}
...
{% block title %}New Post 路 SpaceBox{% endblock %}
{% block content %}
  ...
  <div class="card-body p-2 p-md-3">
    ...
    >
      ...
      <div class="mb-3">
        <div class="d-flex justify-content-between align-items-center mb-2">
          <label class="form-label mb-0 fw-semibold">
            Content <span class="fw-normal text-secondary">(optional)</span>
          </label>
        ...
        <textarea
          ...
          placeholder="Share something... (optional; you can upload only images or videos)&#10;&#10;Tip: pressing Tab here inserts indentation instead of moving focus."
        ></textarea>
      </div>
      ...
      <div class="mb-3">
        ...
        >
        <div class="form-text">
          Up to 6 attachments; images up to 256 MB, videos up to 4.0 GB, and 8.0 GB total. Files are saved with low-memory streaming,
          and large videos support upload progress, seeking, and HTTP Range reads.
        </div>
      ...
      <div class="row g-3 mb-4">
        ...
        <div class="col-md-6">
          ...
          <div class="form-text">
            Uses your browser's local time. Leave blank to publish immediately.
          </div>
        </div>
      </div>
            ...
            <div class="d-flex justify-content-between gap-2 align-items-start">
              ...
              %}
                <form
                  ...
                  onsubmit="return confirm('Delete the text of this comment? Replies below it will be preserved.');"
                >
                  <input type="hidden" name="csrf" value="{{ csrf_token }}">
                  <button
...
{% block title %}Post 路 SpaceBox{% endblock %}
{% block content %}
{% include "partials/post_card.html" %}

{% if not post_is_published %}
  <div class="alert alert-warning border-0 shadow-sm">
    This post is scheduled for future publication and is currently visible only to you.
    At the scheduled time, eligible viewers will be able to see it and comments will open.
  </div>
{% else %}
  <section id="comments" class="comments-section">
    ...
    >
      <div>
        ...
        <p class="text-secondary mb-0">Join the conversation about this post.</p>
      </div>
      <span class="discussion-count small text-secondary">
        {{ post.active_comment_count }} comments
      </span>
    </header>
    ...
    {% if current_user %}
      <details class="comment-composer-disclosure">
        <summary class="comment-composer-toggle">
          ...
          <span class="comment-toggle-open-label">Write a comment</span>
          <span class="comment-toggle-close-label">Hide composer</span>
        </summary>
        ...
        >
          ...
          <textarea
            ...
            required
            data-indentable
            placeholder="Write your thoughts... (Tab inserts indentation)"
          ></textarea>
          <div class="d-flex justify-content-end">
            <button class="btn btn-primary px-4" type="submit">
              Post comment
...
{% block title %}{{ profile_user.display_name }} 路 SpaceBox{% endblock %}
{% block content %}
      ...
      <div class="d-flex gap-3 min-w-0">
        ...
        <div class="min-w-0">
          ...
          <div class="small mt-2">
            <span class="text-secondary">Direct link:</span>
            <a
...
<div class="d-flex justify-content-between align-items-center mb-3">
  ...
  <span class="small text-secondary">
    Profiles are publicly accessible; each post still enforces its own visibility rules.
  </span>
</div>
...
{% else %}
  <div class="empty-state card border-0 shadow-sm text-center text-secondary py-5">
    There are currently no posts on this profile that you can view.
  </div>
{% endif %}
{% endblock %}""",
    "register.html": r"""{% extends "base.html" %}
{% block title %}Sign Up 路 SpaceBox{% endblock %}
{% block content %}
<div class="auth-card card border-0 shadow-sm mx-auto">
  <div class="card-body p-4 p-md-5">
    ...
    <form method="post" action="{{ request.url_for('register') }}">
      ...
      <div class="mb-3">
        ...
        >
        <div class="form-text">3鈥?2 characters; letters, numbers, and underscores only.</div>
      </div>
...
{% block title %}Settings 路 SpaceBox{% endblock %}
{% block content %}
<div class="card border-0 shadow-sm settings-card">
  <div class="card-body p-2 p-md-3">
    <h1 class="h4 mb-1">Account and publishing settings</h1>
    <p class="text-secondary small mb-4">
      Anyone who knows your username can open your profile. Each post can still independently be set to
...
APPLICATION_JAVASCRIPT_TEMPLATE = r"""(() => {
  ...
  function setupCharacterCounters() {
    for (const input of findElements("[data-count-target]")) {
      ...
      function updateCharacterCount() {
        const maximumLength = input.maxLength > 0 ? input.maxLength : "鈭?;
  ...
  function setupLocalizedTimestamps() {
    ...
    for (const timeElement of findElements("time[data-datetime]")) {
      ...
      timeElement.title = `${fullDateTime} (browser local time)`;
  ...
  function setupMediaUpload() {
    ...
    function releasePreviewUrls() {
      // Object URLs must be explicitly revoked so repeatedly selecting large files does not keep consuming memory.
    ...
    function createMediaPreview(file) {
      ...
      if (file.size <= previewLimitBytes) {
        ...
        const largeFileMessage = createElement(
          ...
          "Large file selected; preview is disabled before publishing to save memory.",
        );
      ...
      const caption = createElement(
        ...
        `${file.name} 路 ${formatByteSize(file.size)}`,
      );
    ...
    uploadForm.addEventListener("submit", function submitMediaForm(event) {
      ...
      // Standard form submission cannot report upload progress, so media forms submit the same FormData via XHR.
      event.preventDefault();
      setUploadState(uploadForm, true);
      uploadStatus.container.classList.remove("d-none");
      uploadStatus.progress.value = 0;
      uploadStatus.text.textContent = "Preparing upload...";
      ...
      uploadRequest.upload.addEventListener(
        ...
        function updateUploadProgress(progressEvent) {
          if (!progressEvent.lengthComputable) {
            ...
            uploadStatus.text.textContent = "Uploading large media...";
            return;
          }
          ...
          uploadStatus.text.textContent =
            `Uploading: ${formatByteSize(progressEvent.loaded)} / ` +
      ...
      uploadRequest.upload.addEventListener("load", function finishUpload() {
        ...
        uploadStatus.text.textContent = "Upload complete; validating and saving...";
      });
      uploadRequest.addEventListener("load", function handleUploadResponse() {
        ...
        try {
          ...
          // Keep a generic message for non-JSON error responses.
        }
        uploadStatus.text.textContent = serverDetail
          ? `Upload failed (HTTP ${uploadRequest.status}): ${serverDetail}`
          : `Upload failed (HTTP ${uploadRequest.status}); please try again.`;
      });
      uploadRequest.addEventListener("error", function handleUploadError() {
        setUploadState(uploadForm, false);
        uploadStatus.text.textContent = "The network connection was interrupted before the upload completed. Please try again.";
      });
      uploadRequest.addEventListener("abort", function handleUploadAbort() {
        setUploadState(uploadForm, false);
        uploadStatus.text.textContent = "Upload canceled.";
      });
    ...
    function renderUserSearchResults(results) {
      ...
      if (results.length === 0) {
        searchResultsMenu.append(
          createElement("div", "search-empty", "No similar usernames found"),
        );
      } else {
    ...
    searchInput.addEventListener("input", function scheduleUserSearch() {
      ...
      searchTimerId = window.setTimeout(async function loadUserSearchResults() {
        ...
        try {
          ...
          // Render only the response for the current input so slow requests cannot overwrite newer search results.
...
def build_application_javascript() -> str:
    """Inject server-side media limits into the readable frontend script."""
...
def load_embedded_asset(asset_name: str) -> bytes:
    """Encode SpaceBox first-party frontend assets on demand."""
...
def load_or_create_session_secret(secret_path: Path) -> str:
    """Atomically create or read the persistent session secret."""
    try:
    ...
    try:
        ...
        if len(concurrent_secret) < 32:
            raise RuntimeError("The session secret file is invalid. Delete it and restart.") from exc
...
def configure_sqlite_connection(
    ...
    """Enable foreign keys and a reasonable concurrency wait timeout for each new connection."""
...
def get_database_session() -> Generator[Session, None, None]:
    """Provide a SQLAlchemy Session for each request and always close it."""
...
def commit_database_session(session: Session) -> None:
    """Commit the current transaction and restore Session usability if the commit fails."""
    try:
        session.commit()
    except BaseException:
        # A failed commit must be rolled back explicitly before the Session can be used safely again.
        session.rollback()
        raise
...
class User(Base):
    """A registered user account and profile."""
...
class Post(Base):
    """A user-created post with optional scheduled publication and media attachments."""
    ...
    def active_comment_count(self) -> int:
        """Return the number of comments that have not been soft-deleted."""
        active_count = 0
...
class Media(Base):
    """Index for an image or video attachment stored in the separate media directory."""
    ...
    # Paths are always relative to BASE_DIR; random filenames keep user input out of path resolution.
...
class Comment(Base):
    """A tree-structured post comment with soft-delete support."""
...
class FlashMessage(TypedDict):
    """A flash message stored in the signed session cookie."""
...
def hash_password(password: str) -> str:
    """Return the unsalted SHA-256 hexadecimal digest computed directly from a password."""
    return hashlib.sha256(password.encode("utf-8")).hexdigest()


def verify_password(password: str, encoded_password: str) -> bool:
    """Verify a password against a SHA-256 hexadecimal digest computed directly from the password."""
...
def get_authenticated_user(request: Request, session: Session) -> User | None:
    """Return the authenticated user for the request, if present."""
...
def validate_csrf_token(request: Request, submitted_token: str) -> None:
    """Validate the submitted CSRF token using a constant-time comparison."""
...
class TemplateRenderer:
    """Wrap loading embedded Jinja templates and rendering HTML responses."""
    ...
    def __init__(self, template_map: dict[str, str]) -> None:
        ...
        # Asset URLs include a version parameter so browsers fetch updated CSS and JavaScript after upgrades.
...
async def application_lifespan(_app: FastAPI) -> AsyncIterator[None]:
    """Complete database and media-index maintenance before accepting requests."""
...
def serve_embedded_asset(path: str) -> Response:
    """Serve SpaceBox first-party frontend assets embedded directly in this Python file."""
    try:
...
@dataclass(frozen=True, slots=True)
class StoredMedia:
    """Metadata for an attachment validated and atomically written to the media directory."""
...
@dataclass(frozen=True, slots=True)
class ByteRange:
    """A single HTTP byte range represented as a closed interval."""
...
@dataclass(frozen=True, slots=True)
class MediaSource:
    """A located filesystem media source that can be read in chunks."""
...
class MediaReconciliationReport:
    """Result of reconciling media files with database indexes."""
...
def sanitize_upload_filename(filename: str) -> str:
    """Remove path and control characters, preserving only a safe display filename."""
...
def parse_http_byte_range(range_header: str, total_bytes: int) -> ByteRange:
    """Parse one RFC 7233 byte range; invalid or multi-range requests raise ValueError."""
    ...
    if not start_text and not end_text:
        raise ValueError("Empty Range request")
    ...
    if start_text:
        ...
        if range_start >= total_bytes or range_end < range_start:
            raise ValueError("Range exceeds media size")
        range_end = min(range_end, total_bytes - 1)
    else:
        suffix_length = int(end_text)
        if suffix_length <= 0:
            raise ValueError("Invalid Range suffix length")
...
def _path_depth(path: Path) -> int:
    """Return path depth for removing empty directories from deepest to shallowest."""
    return len(path.parts)


class MediaStorage:
    """Encapsulate validation, atomic storage, streaming reads, and cleanup for large media."""
    ...
    def classify_upload(upload: UploadFile) -> tuple[str, str, MediaFormat]:
        ...
        if media_format is None:
            raise HTTPException(
                ...
                detail=(
                    ...
                    "Only JPG/PNG/GIF/WebP images and MP4/WebM/MOV/M4V videos are allowed."
                ),
            )
        ...
        if (
            ...
            raise HTTPException(
                status_code=status.HTTP_415_UNSUPPORTED_MEDIA_TYPE,
                detail=f"File {original_name} has an extension that does not match its media type.",
            )
        return original_name, extension, media_format

    @staticmethod
    def matches_file_signature(extension: str, header: bytes) -> bool:
        """Check common media magic bytes instead of relying only on the client-provided MIME type."""
    ...
    def store_upload_stream(
        ...
        """Synchronously copy an upload stream in chunks; called from a thread pool to avoid blocking the event loop."""
        ...
        try:
            ...
            if not self.matches_file_signature(extension, first_chunk):
                raise HTTPException(
                    status_code=status.HTTP_415_UNSUPPORTED_MEDIA_TYPE,
                    detail=f"File {original_name} content does not match its extension.",
                )
            ...
            # Atomic replacement ensures media indexes never point to a partially written destination file.
            ...
            if exc.errno in {errno.ENOSPC, errno.EDQUOT, errno.EFBIG}:
                raise HTTPException(
                    status_code=507,
                    detail="Not enough disk space to save this media file.",
                ) from exc
            raise
        except BaseException:
            temporary_path.unlink(missing_ok=True)
    ...
    async def store_uploads(self, uploads: list[UploadFile]) -> list[StoredMedia]:
        """Save a batch of uploads sequentially and roll back files already written if any item fails."""
        ...
        try:
            ...
            if declared_total_bytes > MAX_MEDIA_TOTAL_SIZE_BYTES:
                raise HTTPException(
                    status_code=status.HTTP_413_CONTENT_TOO_LARGE,
                    detail=(
                        "Total attachment size for one post must not exceed "
    ...
    def resolve_indexed_path(self, stored_path: str) -> Path | None:
        """Safely resolve a media index, accepting only regular paths within the media root."""
        if not stored_path:
            return None
        candidate_path = (self.application_root / stored_path.lstrip("/")).resolve()
        try:
            # relative_to also acts as a bounds check, rejecting directory traversal and indexes outside the media root.
    ...
    def delete_indexed_files(self, stored_paths: Iterable[str]) -> None:
        """Best-effort removal of media files; database transactions do not roll back because cleanup fails."""
    ...
    def reconcile_index(self, session: Session) -> MediaReconciliationReport:
        """Reconcile media indexes, file metadata, and the media directory at startup."""
        ...
        for media_record in session.scalars(select(Media)).all():
            ...
            try:
                if (
                    ...
                    raise OSError("Media index points to an invalid file")
                ...
                if file_stat.st_size <= 0 or not self.matches_file_signature(
                    ...
                    raise OSError("Invalid media content signature")
            except OSError:
    ...
    def load_media_source(self, media_record: Media) -> MediaSource:
        """Locate a stream-readable media file from its database index."""
        resolved_path = self.resolve_indexed_path(media_record.file_path)
    ...
    def iter_file_range(
        ...
        """Read only the requested range using a fixed-size buffer."""
    ...
    def build_media_response(
        ...
        """Build a low-memory media response supporting caching, HEAD, and a single Range."""
        common_headers = {
            "Accept-Ranges": "bytes",
        ...
        if range_header and request.headers.get("if-range", source.etag) == source.etag:
            try:
                # Video seeking depends on Range; parse only one range to keep responses predictable.
...
class StartupMaintenanceReport:
    """Summary of database initialization, stale-account cleanup, and media reconciliation."""
    ...
    def summary(self) -> str:
        return (
            ...
            f"stale empty accounts removed: {self.stale_empty_accounts_removed}; "
            f"invalid media indexes removed: {self.media.invalid_records_removed}; "
            f"media index metadata corrected: {self.media.metadata_records_updated}; "
            f"orphan files removed: {self.media.orphan_files_removed}; "
            f"interrupted-upload files removed: {self.media.partial_files_removed}."
        )
...
def cleanup_stale_empty_accounts(
    ...
    """Delete empty accounts that have not logged in for 30 days and have no profile data or interactions."""
    cutoff_time = (current_time or utc_now()) - timedelta(days=STALE_EMPTY_ACCOUNT_DAYS)
    # An 鈥渆mpty account鈥?must have no bio, content, or follow relationships to avoid deleting users who still have data.
...
def initialize_application_state() -> StartupMaintenanceReport:
    """Create a fresh database schema and perform the safety maintenance required at every startup."""
...
def normalize_line_endings(value: str) -> str:
    """Normalize line endings while preserving indentation and leading/trailing spaces."""
...
def parse_scheduled_datetime(value: str | None) -> datetime | None:
    """Parse the browser-provided scheduled-publication timestamp and normalize it to UTC."""
    if not value:
        return None
...
def publish_due_posts(session: Session) -> None:
    """Mark scheduled posts as published once their scheduled time arrives."""
    current_time = utc_now()
...
def is_post_published(post: Post) -> bool:
    """Return whether the post should currently be considered published."""
...
def get_followed_user_ids(
    ...
    """Return the IDs of accounts followed by the current user."""
    if current_user is None:
        return set()
...
def can_view_post(
    ...
    """Evaluate a post's scheduled-publication and visibility rules."""
...
def build_post_select() -> Select[tuple[Post]]:
    """Build the shared post query and preload lightweight media indexes."""
...
def comment_sort_key(comment: Comment) -> tuple[datetime, int]:
    """Return a stable comment-time sort key."""
...
def build_comment_tree(comments: list[Comment]) -> list[CommentTreeNode]:
    """Build the O(n) comment tree while retaining soft-deleted parent comments."""
...
@dataclass(frozen=True, slots=True)
class PostDraft:
    """Post input after text, visibility, and publication-time validation."""
...
def build_post_draft(
    *,
    ...
    """Centralize post-form validation so the route only coordinates authentication, storage, and response handling."""
    normalized_content = normalize_line_endings(content or "")
    if not normalized_content.strip() and upload_count == 0:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="A post must contain text or at least one attachment.",
        )
    if len(normalized_content) > MAX_POST_LENGTH:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Post content cannot exceed 5,000 characters.",
        )
    if upload_count > MAX_MEDIA_FILES_PER_POST:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Each post can contain at most {MAX_MEDIA_FILES_PER_POST} attachments.",
        )
...
def persist_post(
    ...
    """Persist the post and attachment metadata in a single database transaction."""
    new_post = Post(
        author_id=author_id,
...
def calculate_lcs_length(left: str, right: str) -> int:
    """Return the longest common subsequence length using one-dimensional dynamic programming."""
...
def calculate_username_search_score(
    ...
    """Compute an LCS-based username similarity score and match length."""
    ...
    # LCS remains the primary signal. Prefix and substring matches are used only to make rankings more intuitive when usernames have similar LCS values
    # and would otherwise be difficult to distinguish.
...
def filter_visible_posts(
    ...
    """Filter posts by viewing permission and optionally stop once the result limit is reached."""
...
class UsernameSearchMatch:
    """A username search candidate with a precomputed similarity score."""
...
def username_search_sort_key(
    ...
    """Rank more relevant, shorter, and lexicographically earlier usernames first."""
    return (
        -search_match.score,
...
def serialize_user_search_matches(
    ...
    """Convert backend search results to the camelCase JSON shape used by the frontend."""
...
async def prepare_upload_files(
    ...
    """Keep upload items that have filenames and promptly close empty items generated by the browser."""
...
def register(
    ...
    if not username_length_is_valid or not username_characters_are_valid:
        set_flash_message(
            request, "Username must be 3鈥?2 characters and may contain only letters, numbers, or underscores.", "danger"
        )
    ...
    if (
        ...
        set_flash_message(request, "Display name is required and cannot exceed 64 characters.", "danger")
        return redirect_to("/register")
    if session.scalar(select(User).where(User.username == normalized_username)):
        set_flash_message(request, "That username is already taken.", "danger")
        return redirect_to("/register")
    ...
    set_flash_message(request, "Registration complete. Welcome to SpaceBox!", "success")
    return redirect_to("/")
...
def login(
    ...
    if authenticated_user is None or not verify_password(
        ...
        set_flash_message(request, "Incorrect username or password.", "danger")
    ...
    get_or_create_csrf_token(request)
    set_flash_message(request, "Logged in successfully.", "success")
    return redirect_to("/")
...
async def create_post(
    ...
    if draft.scheduled_at:
        set_flash_message(request, "Post scheduled successfully.", "success")
    else:
        set_flash_message(request, "Post published successfully.", "success")
...
def create_comment(
    ...
    if not normalized_content.strip() or len(normalized_content) > MAX_COMMENT_LENGTH:
        set_flash_message(request, "Comment cannot be empty and cannot exceed 1,000 characters.", "danger")
    ...
    commit_database_session(session)
    set_flash_message(request, "Comment posted successfully.", "success")
...
def delete_comment(
    ...
    if not comment.is_deleted:
        ...
        # Soft deletion intentionally preserves content and parent_id in the database.
        commit_database_session(session)
        set_flash_message(
            request,
            "Comment marked as deleted; replies beneath it were preserved.",
            "success",
        )
...
def delete_post(
    ...
    commit_database_session(session)
    media_storage_service.delete_indexed_files(stored_paths)
    set_flash_message(request, "Post deleted successfully.", "success")
    return redirect_to("/")
...
def render_profile_page(
    ...
    """Render the public profile shell and filter posts according to viewer permissions."""
    publish_due_posts(session)
...
def follow_user(
    ...
    set_flash_message(request, f"Now following @{target_user.username}", "success")
...
def unfollow_user(
    ...
    commit_database_session(session)
    set_flash_message(request, f"Unfollowed @{target_user.username}", "success")
    return redirect_to(f"/u/{target_user.username}")
...
def settings_update(
    ...
    if len(normalized_bio) > MAX_BIO_LENGTH:
        set_flash_message(request, "Bio cannot exceed 500 characters.", "danger")
        return redirect_to("/settings")
    ...
    set_flash_message(request, "Settings saved successfully.", "success")
    return redirect_to("/settings")
...
def _find_available_port(start_port: int = 8000, attempts: int = 50) -> int:
    """Find an available TCP port on all network interfaces, preferring port 8000."""
    for port in range(start_port, start_port + attempts):
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as probe_socket:
            try:
                ...
                continue
            return port
    raise RuntimeError("No available local port was found in the range 8000鈥?049.")


def main() -> None:
    """Start SpaceBox without CLI arguments or environment configuration."""
    host = "0.0.0.0"
    ...
    print(f"Media dir  : {MEDIA_STORAGE_DIR}")
